...
parser.add_option('-a', '--addr', default='-1', help='请求列表（逗号分隔）[-1->使用addrDesc]', action='store', type='string',
                  dest='addr')
parser.add_option('-A', '--addrDesc', default='5,-1,0', help='请求数量、max请求（-1->all）、min请求', action='store',
                  type='string', dest='addrDesc')
parser.add_option('-S', '--seekSpeed', default='1', help='寻道速度', action='store', type='string', dest='seekSpeed')
parser.add_option('-R', '--rotSpeed', default='1', help='旋转速度', action='store', type='string', dest='rotateSpeed')
parser.add_option('-p', '--policy', default='FIFO', help='调度策略【FIFO、SSTF、SATF、BSATF】', action='store', type='string',
                  dest='policy')
parser.add_option('-w', '--schedWindow', default=-1, help='计划窗口大小（-1->全部）', action='store', type='int', dest='window')
parser.add_option('-o', '--skewOffset', default=0, help='倾斜量（以块为单位）', action='store', type='int', dest='skew')
parser.add_option('-z', '--zoning', default='30,30,30', help='外、中、内轨道上的块之间的角度', action='store', type='string',
                  dest='zoning')
parser.add_option('-G', '--graphics', default=False, help='打开图形', action='store_true', dest='graphics')
parser.add_option('-l', '--lateAddr', default='-1', help='延迟：请求列表（逗号分隔）[-1->随机]', action='store', type='string',
                  dest='lateAddr')
parser.add_option('-L', '--lateAddrDesc', default='0,-1,0', help='Num请求、max请求（-1->all）、min请求',
                  action='store', type='string', dest='lateAddrDesc')
parser.add_option('-c', '--compute', default=False, help='计算结果', action='store_true', dest='compute')
(options, args) = parser.parse_args()

print('选项 随机种子', options.seed)
print('选项 请求列表', options.addr)
print('选项 请求描述', options.addrDesc)
print('选项 寻道速度', options.seekSpeed)
print('选项 旋转速度', options.rotateSpeed)
print('选项 倾斜量', options.skew)
print('选项 计划窗口大小', options.window)
print('选项 调度策略', options.policy)
print('选项 计算结果', options.compute)
print('选项 是否作图', options.graphics)
print('选项 轨道上的块之间的角度', options.zoning)
print('选项 延迟请求列表', options.lateAddr)
print('选项 延迟请求描述', options.lateAddrDesc)
print('')
