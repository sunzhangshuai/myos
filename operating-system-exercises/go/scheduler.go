...
func main() {
	...
	if *jlist != "" {
		...
		rander := rand.New(rand.NewSource(*seed3))
...
func fifo(joblist Jobs) {
	...
	for _, job := range joblist {
		fmt.Printf("[ time %3d ] Run job %d for %3d secs ( DONE at %3d )\n", thetime, job.idx, job.len, thetime+job.len)
		thetime += job.len
	}
